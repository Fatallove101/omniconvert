...
> **歌曲转换的边界**：本工具做的是"解密还原"——去掉加密壳得到**原本就封装在内**的 MP3/FLAC/OGG（音质无损），**不做有损转码**（如 FLAC→MP3 需要音频编码器 ffmpeg，本项目不内置音频编码器）。QQ 音乐新版 MFLAC/MGG 有两种：页脚**内嵌明文 eKey** 的（直接解密）；以及页脚为 **`musicex` 结构、文件里不含 eKey** 的——这类文件的密钥只存在于 QQ 音乐客户端运行期，离线拿不到，需要你粘贴该曲 eKey 才能解密（否则只会明确报错，不会输出打不开的文件）。酷我 KWM v2 需要单独提取的密钥，暂不支持。仅限解密你拥有合法权利的个人歌曲文件。
...
- QQ 音乐新版 mgg/mflac（页脚 `musicex` 结构）文件里**不含明文 eKey**：能否解密取决于你有没有该曲 eKey；没有时会明确报错说明原因，不会像以前那样输出一个打不开的文件
